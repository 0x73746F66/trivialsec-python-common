...
class Metadata:
    ...
    def fetch(self, verify_tls :bool = False, allow_redirects :bool = False, http_timeout: int = 3):
        ...
        if isinstance(self.server_certificate, X509) and self._json_certificate == '{}':            
            try:
                self._json_certificate = json.dumps(ssl._ssl._test_decode_cert(StringIO(dump_certificate(FILETYPE_PEM, self.server_certificate))), default=str) # pylint: disable=protected-access
