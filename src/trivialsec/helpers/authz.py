...
def start_transaction(target :str) -> str:
    ...
    config.redis_client.set(cache_key, secret_key, ex=timedelta(seconds=int(config.authz_expiry_seconds)))
...
def get_authorization_token(mfa_key :str, transaction_id :str) -> str:
    ...
    config.redis_client.set(cache_key, transaction_id, ex=timedelta(seconds=int(config.authz_expiry_seconds)))
    return authorization_token

def is_active_transaction(transaction_id :str, target :str) -> bool:
    cache_key_kid = f'{config.app_version}{transaction_id}'
    stored_value = config.redis_client.get(cache_key_kid)
    if stored_value is None:
        return False
...
def verify_transaction(mfa_key : str, target :str, authorization_token :str) -> bool:
    ...
    stored_value = config.redis_client.get(cache_key_tid)
    if stored_value is None:
